...
test("external counsel and operator identities remain distinct and approval is immutable",async()=>{
  ...
  await assert.rejects(workflow.approveClassification({id:draft.id,counselName:"External Counsel",counselLawFirm:"Test Firm",approvalReference:"TEST-ONLY",sourceReference:"TEST-EVIDENCE",approvalDate:"2032-05-01",effectiveFrom:new Date("2032-05-01"),actorAdminId:adminId,confirmationPhrase:GOVERNANCE_APPROVAL_PHRASE,immutableAcknowledged:true}),/approval_rejected/);
  await workflow.submitClassification({id:draft.id,actorAdminId:adminId});
  await workflow.approveClassification({id:draft.id,counselName:"External Counsel",counselLawFirm:"Test Firm",approvalReference:"TEST-ONLY",sourceReference:"TEST-EVIDENCE",approvalDate:"2032-05-01",effectiveFrom:new Date("2032-05-01"),actorAdminId:adminId,confirmationPhrase:GOVERNANCE_APPROVAL_PHRASE,immutableAcknowledged:true});
  const row=(await db.query(`SELECT counsel_name,entered_by_admin_id::text,status FROM signature_document_type_approvals WHERE id=$1`,[draft.id])).rows[0];
  assert.deepEqual(row,{counsel_name:"External Counsel",entered_by_admin_id:adminId,status:"approved"});
  await assert.rejects(db.query(`UPDATE signature_document_type_approvals SET counsel_name='changed' WHERE id=$1`,[draft.id]),/immutable/);
});
...
test("consent and privacy require review and exact immutable hashes",async()=>{
  ...
  await assert.rejects(workflow.approveConsent({id:consent.id,approvalReference:"TEST",externalReviewerName:"Reviewer",externalReviewerReference:"REF",effectiveFrom:new Date("2032-05-01"),actorAdminId:adminId,confirmationPhrase:"wrong",immutableAcknowledged:true}),/confirmation_required/);
  await workflow.approveConsent({id:consent.id,approvalReference:"TEST",externalReviewerName:"Reviewer",externalReviewerReference:"REF",effectiveFrom:new Date("2032-05-01"),actorAdminId:adminId,confirmationPhrase:GOVERNANCE_APPROVAL_PHRASE,immutableAcknowledged:true});
  ...
  await workflow.approvePrivacy({id:privacy.id,approvalReference:"TEST",externalReviewerName:"Reviewer",externalReviewerReference:"REF",effectiveFrom:new Date("2032-05-01"),actorAdminId:adminId,confirmationPhrase:GOVERNANCE_APPROVAL_PHRASE,immutableAcknowledged:true});
...
test("retention approval and activation are separate and preview never deletes",async()=>{
  ...
  await workflow.approveRetention({id:retention.id,approvalReference:"TEST",externalReviewerName:"Reviewer",externalReviewerReference:"REF",actorAdminId:adminId,confirmationPhrase:GOVERNANCE_APPROVAL_PHRASE,immutableAcknowledged:true});
...
test("governance mutations are server-authenticated and reject replayed or stale approval",async()=>{
  ...
  await workflow.submitConsent({id:consent.id,actorAdminId:adminId});
  const approval={id:consent.id,approvalReference:"TEST",externalReviewerName:"Reviewer",externalReviewerReference:"REF",effectiveFrom:new Date("2032-05-01"),actorAdminId:adminId,confirmationPhrase:GOVERNANCE_APPROVAL_PHRASE,immutableAcknowledged:true};
