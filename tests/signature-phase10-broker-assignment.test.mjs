...
test("broker candidates resolve through a narrow trusted compatibility source", async () => {
  ...
  const candidates = await listSignatureBrokerCandidates({
    unsafe: async () => [{ id: "broker-1", name: "Ivonne Erickson", email: "IVONNE@example.test" }],
  });
  assert.deepEqual(candidates, [{ id: "broker-1", name: "Ivonne Erickson", email: "ivonne@example.test" }]);
  assert.deepEqual(
  ...
  const multiple = [...candidates, { id: "broker-2", name: "Otro corredor", email: "otro@example.test" }];
  assert.equal(
...
test("new documents own the broker assignment while legacy settings remain a candidate fallback", async () => {
