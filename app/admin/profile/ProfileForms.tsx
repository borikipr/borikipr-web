...
export function RolePicker({ roles, onChange, customTitle, onCustomTitleChange }: {
  ...
  return <div className="profile-role-picker">
    ...
    <div ref={pickerRef} className={`profile-role-control ${open ? "is-open" : ""}`} style={{ marginTop: ".5rem", position: "relative" }}>
      ...
      <button type="button" className="profile-role-toggle" style={{ alignItems: "center", borderRadius: ".375rem", color: "#4d4d4d", display: "inline-flex", height: "2.5rem", justifyContent: "center", position: "absolute", right: ".25rem", top: ".25rem", width: "2.5rem" }} onClick={() => setOpen((value) => !value)} aria-label={open ? "Cerrar opciones de roles profesionales" : "Abrir opciones de roles profesionales"} aria-controls="professional-role-options" aria-expanded={open} disabled={roles.length >= 2}>{open ? <ChevronUp aria-hidden="true" size={18}/> : <ChevronDown aria-hidden="true" size={18}/>}</button>
      {open && <div id="professional-role-options" role="listbox" aria-label="Opciones de roles profesionales" className="profile-role-options" style={rolePanelStyle}>
        {filtered.length ? filtered.map((role, index) => { const selected = roles.includes(role.id); const disabled = !selected && roles.length >= 2; return <button key={role.id} id={`professional-role-${role.id}`} type="button" role="option" aria-selected={selected} disabled={disabled} className={index === activeIndex ? "is-active" : ""} style={{ alignItems: "center", borderRadius: ".375rem", color: selected || index === activeIndex ? "#0d406d" : "#000000", display: "flex", flex: "0 0 auto", fontSize: ".875rem", gap: ".75rem", justifyContent: "space-between", minHeight: "2.5rem", padding: ".5rem .75rem", textAlign: "left", width: "100%", ...(selected || index === activeIndex ? { background: "#f8f8f8" } : {}) }} onMouseDown={(event) => event.preventDefault()} onClick={() => toggle(role.id)}><span>{role.label}</span>{selected ? <BadgeCheck aria-label="Seleccionado" size={16}/> : null}</button>; }) : <p className="px-3 py-2 text-sm text-slate-500">No encontramos ese rol.</p>}
      </div>}
    </div>
...
export default function ProfileForms({ displayName, professionalTitle, professionalRoles, professionalLicenseNumber, profileImageUrl, email, username, roleLabel, professionalEmail, professionalPhone, whatsappEnabled, professionalBio, publicProfileEnabled, publicProfileApprovalState }: {
  ...
  const [whatsappEnabledOverride, setWhatsappEnabledOverride] = useState<boolean | null>(null);
  ...
  const whatsappEnabledState = whatsappEnabledOverride
    ?? profileState.professionalPhoneWhatsappEnabled
    ?? whatsappEnabled;
