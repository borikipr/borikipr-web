...
export default async function UseSignatureTemplatePage({params}:{params:Promise<{id:string}>}){
  const session=await getAdminSession();if(!session)redirect("/admin/login");await requireModuleAccess("signatures","manage");const {id}=await params;
  ...
  const brokerCandidates=template.requiresBrokerSignature?await listSignatureBrokerCandidates(database,session.id):[];
