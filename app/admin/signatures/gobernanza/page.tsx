...
export default async function SignatureGovernancePage() {
  ...
  const governanceDrafts = {
    ...
    consents: consents.map((row) => ({ id: row.id, label: `${row.locale} · ${row.version_identifier} · ${row.status}`, status: row.status })),
    privacy: privacy.map((row) => ({ id: row.id, label: `${row.version_identifier} · ${row.status}`, status: row.status })),
