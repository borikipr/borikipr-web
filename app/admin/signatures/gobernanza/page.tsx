...
import { SectionHeader, SummaryCard } from "@/components/admin/AdminUI";
...
export default async function SignatureGovernancePage() {
  ...
  return <AdminPageShell>
    ...
    <nav aria-label="Secciones de gobernanza" className="surface-card flex flex-wrap gap-1.5 p-2 text-sm font-semibold">
      {[['estado','Estado'],['documentos','Controles'],['retencion','Retención'],['governance-management','Cambios y autorizaciones'],['referencia','Auditoría y referencia']].map(([id,label])=><a className="shrink-0 rounded-lg px-3 py-2 text-slate-700 hover:bg-slate-100" href={`#${id}`} key={id}>{label}</a>)}
    </nav>

    <section className="surface-card scroll-mt-24 overflow-hidden" id="estado" aria-labelledby="estado-operativo">
      <SectionHeader title="Estado operativo" description="La salud actual de Firmas se verifica contra la puerta pública canónica." action={<StatusBadge variant={readinessVariant(publicGateState)}>{publicLaunchGate.allowed ? "Saludable" : "Requiere atención"}</StatusBadge>} />
      <div className="grid divide-y divide-slate-100 sm:grid-cols-2 sm:divide-x sm:divide-y-0 xl:grid-cols-5">
        <article className="p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Firma pública</p><p className="mt-2 font-semibold text-slate-950">{publicLaunchGate.allowed ? "Activa" : publicFlag ? "Habilitada · bloqueada" : "Desactivada"}</p><p className="mt-1 text-sm text-slate-600">{publicLaunchGate.allowed ? "Activación pública válida." : "La puerta canónica impide el acceso público."}</p></article>
        <article className="p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Canary interno</p><p className="mt-2 font-semibold text-slate-950">{internalCanaryEnabled ? "Activo con validación" : "Desactivado"}</p><p className="mt-1 text-sm text-slate-600">Permanece separado de la firma pública.</p></article>
        <article className="p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Neon recovery</p><p className="mt-2 font-semibold text-slate-950">{neonRecoveryProven ? "Listo" : "Bloqueado"}</p><p className="mt-1 text-sm text-slate-600">{neonRecoveryProven ? "Recuperación registrada." : "Falta una prueba válida."}</p></article>
        <article className="p-4"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">R2 recovery</p><p className="mt-2 font-semibold text-slate-950">{r2RecoveryProven ? "Listo" : "Bloqueado"}</p><p className="mt-1 text-sm text-slate-600">{r2RecoveryProven ? "Recuperación independiente registrada." : "Falta una prueba válida."}</p></article>
        <article className="p-4 sm:col-span-2 xl:col-span-1"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">Readiness</p><p className="mt-2 font-semibold text-slate-950">{publicLaunchGate.allowed ? "Lanzamiento válido" : "Revisión necesaria"}</p><p className="mt-1 text-sm text-slate-600">{publicLaunchGate.allowed ? "Sin bloqueadores públicos." : "Revise los bloqueadores canónicos abajo."}</p></article>
      </div>
      {!publicLaunchGate.allowed && <div className="border-t border-red-100 bg-red-50 px-5 py-4"><p className="font-semibold text-red-900">Bloqueadores de lanzamiento</p><ul className="mt-2 space-y-1 text-sm text-red-800">{publicLaunchGate.blockers.map((blocker)=><li key={blocker}>{BLOCKERS[blocker] ?? "Hay un control canónico pendiente."}</li>)}</ul></div>}
    </section>

    <section className="surface-card overflow-hidden">
      <SectionHeader title="Readiness interno" description="Resumen para un canary controlado. Preparado no significa habilitado." action={<StatusBadge variant={readiness.spanishCanaryReady ? "green" : "red"}>{readiness.spanishCanaryReady ? "Listo" : "Bloqueado"}</StatusBadge>} />
      <div className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_20rem]">
        ...
        <div className="rounded-xl bg-slate-50 p-4 text-sm"><p className="font-semibold text-slate-950">Alcance bilingüe</p><p className="mt-1 text-slate-600">Incluir inglés requiere también consentimiento y privacidad en en-US.</p>{readiness.bilingualCanaryBlockers.length ? <ul className="mt-3 space-y-1 text-slate-600">{readiness.bilingualCanaryBlockers.map((blocker)=><li key={blocker}>{BLOCKERS[blocker] ?? "Control pendiente."}</li>)}</ul> : <p className="mt-3 text-emerald-800">Sin bloqueadores adicionales para este alcance.</p>}</div>
      </div>
    </section>
    ...
    <section id="referencia" className="surface-card scroll-mt-24 p-5" aria-labelledby="auditoria-referencia"><SectionHeader title="Auditoría y referencia" description="Matrices, monitoreo y procedimientos para revisión o soporte; no cambian la operación diaria." /><div className="mt-5"><section><h2 id="auditoria-referencia" className="font-semibold text-slate-950">Matriz de preparación</h2><div className="mt-3 overflow-x-auto"><table className="admin-table min-w-[760px]"><thead><tr><th>Control</th><th>Estado</th><th>Responsable</th><th>Evidencia</th></tr></thead><tbody>{launchChecks.map((item)=><tr key={item.label}><td className="font-medium">{item.label}</td><td><StatusBadge variant={readinessVariant(item.state)}>{READINESS_LABELS[item.state]}</StatusBadge></td><td>{item.owner}</td><td className="text-slate-600">{item.evidence}</td></tr>)}</tbody></table></div></section><section className="mt-6 border-t border-slate-100 pt-6"><h2 className="font-semibold text-slate-950">Monitoreo agregado</h2><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">{Object.entries(monitoring).map(([label,value])=><SummaryCard key={label} label={label.replaceAll("_"," ")} value={value}/>)}</dl></section><section className="mt-6 border-t border-slate-100 pt-6"><h2 className="font-semibold text-slate-950">Procedimientos de soporte</h2><div className="mt-3 grid gap-3 md:grid-cols-2">{SUPPORT.map(([title,procedure])=><article className="rounded-xl bg-slate-50 p-4" key={title}><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm text-slate-600">{procedure}</p></article>)}</div></section></div></section>
  </AdminPageShell>;
}
