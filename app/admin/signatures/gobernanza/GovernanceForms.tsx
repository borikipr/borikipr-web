...
export function GovernanceForms({ drafts }: { drafts: Drafts }) {
  ...
  return <section className="signature-governance min-w-0 max-w-full space-y-6" aria-labelledby="governance-management">
    ...
    <details className="surface-card min-w-0 max-w-full overflow-hidden p-4 sm:p-5"><summary className="font-semibold">Autorización — LANZAMIENTO PÚBLICO</summary><p className="mt-3 rounded-lg bg-slate-50 p-3 text-sm">Registra un snapshot global de clasificaciones ordinarias, consentimiento es-PR, privacidad, retención, recuperación e integridad. Autorizar no activa la bandera pública.</p><form action={publicAuthorizationAction} className={`${formClass} mt-4`}><label>Notas de lanzamiento opcionales<textarea name="notes" /></label><label className="flex items-start gap-2"><input name="explicitConfirmation" type="checkbox" value="true" required /><span>Confirmo que revisé el readiness canónico y que esta acción no activa todavía la firma pública.</span></label><label>Escriba <code>{PUBLIC_LAUNCH_CONFIRMATION_PHRASE}</code><input name="confirmationPhrase" required /></label><Submit>Registrar autorización pública</Submit><Result state={publicAuthorization} /></form></details>
