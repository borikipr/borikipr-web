...
import { signatureDeliveryLabel, signatureEventLabel, signatureOperationalStatus, signatureRequiresAttention, signatureStatusLabel, signatureStatusTone } from "@/lib/signatures/admin-ux";
...
export default async function SignatureDraftPage({params}:{params:Promise<{id:string}>}) {
  ...
  const latestDelivery=detail.participants.map((participant)=>participant.lastDeliveryStatus).find((status)=>status==="failed")??detail.participants.find((participant)=>participant.lastDeliveryStatus)?.lastDeliveryStatus;
  const attention=signatureRequiresAttention({status:detail.status,deliveryStatus:latestDelivery,expiresAt:detail.expiresAt});
  const operationalStatus=signatureOperationalStatus({status:detail.status,participants:detail.participants,deliveryStatus:latestDelivery,expiresAt:detail.expiresAt});
  const finalCompletedAt=detail.participants.map((participant)=>participant.completedAt).filter((value):value is string|Date=>Boolean(value)).sort((left,right)=>new Date(right).getTime()-new Date(left).getTime())[0];
  ...
  return <AdminPageShell>
    ...
    {attention?<section className="rounded-xl border border-red-200 bg-red-50 p-4 text-red-900"><h2 className="font-semibold">Requiere atención</h2><p className="mt-1 text-sm">{latestDelivery==="failed"?"La última invitación no pudo entregarse. Revisa el destinatario antes de reenviar.":"La solicitud expiró antes de completarse."}</p></section>:null}
    <section className="surface-card p-5" aria-label="Resumen del documento">
      <div className="flex flex-wrap items-start justify-between gap-3"><div><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-semibold ${signatureStatusTone(detail.status)}`}>{signatureStatusLabel(detail.status)}</span><p className="mt-2 text-lg font-semibold">{operationalStatus}</p></div>{detail.status==="completed"&&finalCompletedAt?<p className="text-sm text-slate-600">Completado {new Date(finalCompletedAt).toLocaleString("es-PR")}</p>:null}</div>
      <dl className="admin-meta-grid mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><div><dt>Enviado por</dt><dd>{detail.createdByName}</dd></div><div><dt>Documento</dt><dd>{detail.version.pageCount} páginas · {(detail.version.byteCount/1024).toFixed(1)} KB</dd></div><div><dt>Firmas</dt><dd>{detail.participants.filter((participant)=>participant.status==="completed").length} de {detail.participants.length}</dd></div><div><dt>Expiración</dt><dd>{detail.expiresAt?new Date(detail.expiresAt).toLocaleDateString("es-PR"):"Sin fecha"}</dd></div></dl>
    </section>

    {detail.status==="completed"&&<section className="surface-card flex flex-wrap gap-3 p-5"><Link className="btn-primary" href={`/admin/signatures/${detail.id}/final`}>Descargar documento firmado</Link><Link className="btn-secondary" href={`/admin/signatures/${detail.id}/certificate`}>Descargar certificado</Link></section>}
    {detail.participants.length?<section className="grid gap-4 lg:grid-cols-2"><SignatureRoutingSummary mode={detail.routingMode} participants={detail.participants}/><section className="surface-card p-5"><h2 className="font-semibold">Destinatarios</h2><ul className="mt-4 divide-y divide-slate-100">{detail.participants.map((participant)=><li className="grid gap-1 py-3 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center" key={participant.id}><div><p className="font-semibold">{participant.name} · {participant.role}</p><p className="text-xs text-slate-600">Etapa {participant.routingOrder??1}{participant.isBrokerFinalSigner?" · Firma final":""}</p></div><div className="text-sm sm:text-right"><p>{participant.status==="completed"?"Firma completada":participant.status==="viewed"?"Documento visto":participant.status==="consented"?"Consentimiento aceptado":participant.status==="invited"?"Invitación enviada":"En espera"}</p><p className="text-xs text-slate-500">{signatureDeliveryLabel(participant.lastDeliveryStatus)}</p></div></li>)}</ul></section></section>:null}
