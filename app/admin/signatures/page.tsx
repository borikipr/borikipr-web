  ...
  return (
    <AdminPageShell>
      ...
      <section aria-label="Lista de solicitudes" className="grid gap-4">
        {rows.length===0 ? <EmptyState title={view==="archived"?"No hay solicitudes archivadas":"Todavía no hay solicitudes en esta vista"} description="Comienza con un PDF y añade quién debe firmar." action={view==="active"?<Link className="btn-primary" href="/admin/signatures/nuevo">Nuevo documento</Link>:undefined}/> : rows.map((row)=><article className="surface-card grid gap-3 px-4 py-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-center md:px-5" key={row.id}>
