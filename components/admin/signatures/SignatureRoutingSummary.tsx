import {
  buildSignatureRoutingStages,
  signatureRoutingModeLabel,
  type SignatureRoutingParticipant,
} from "@/lib/signatures/routing-ux";
...
export default function SignatureRoutingSummary({
  ...
  return (
    <section
      aria-labelledby="signature-routing-heading"
      className={`signature-routing-summary ${compact ? "is-compact" : ""}`}
    >
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h3 id="signature-routing-heading" className="font-semibold">
          Ruta de firmas
        </h3>
      ...
      {!stages.length ? (
        <p className="mt-3 text-sm text-slate-600">
          Añade destinatarios para definir la ruta.
        </p>
      ) : (
        <ol
          className="signature-routing-stages"
          aria-label={`${stages.length} etapas de firma`}
        >
          {stages.map((stage, index) => (
            <li className="signature-routing-stage" key={stage.order}>
              <span
                className="signature-routing-stage-number"
                aria-hidden="true"
              >
                {index + 1}
              </span>
