...
export default function SignatureDocumentActions({ documentId, title, status, operationallyHidden, sourceAvailable, deletionEligible, deletionMode, detailHref }: {
  ...
  return <section id="acciones" aria-label="Acciones de la solicitud" className="flex justify-end">
    <SignatureActionsMenu>
      ...
      <div className="signature-actions-separator"><SignatureMenuItem danger icon={<Trash2 size={17}/>} onSelect={() => setDialog("delete")} disabled={!actions.has("delete")} title={!actions.has("delete") ? "No se puede eliminar porque contiene evidencia o una dependencia protegida." : undefined}>Eliminar definitivamente</SignatureMenuItem></div>
    </SignatureActionsMenu>
